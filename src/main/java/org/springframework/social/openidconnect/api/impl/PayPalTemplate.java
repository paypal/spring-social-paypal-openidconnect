...
public class PayPalTemplate extends AbstractOAuth2ApiBinding implements PayPal {
	...
	public PayPalTemplate(String accessToken, String userInfoUrl,
		...
		if (logger.isDebugEnabled()) {
			logger.debug("user info url is " + userInfoUrl
					+ " and host name verifier isStrict = " + isStrict);
		}
		// could not override request factory using configureRestTemplate method
		// as isStrict instance variable is not
		// initialized before constructor completes. So setting request factory
		// just before constructor completes
	...
	public PayPalProfile getUserProfile() {
		...
		try {
			...
			// Password cannot be blank for Spring security. Setting it to
			// access token rather keeping it as "N/A".
			...
			logger.error("User info could not be retrieved " + ex.getMessage()
	...
	/*
	 * This validation is required to check mandatory field/s being returned by
	 * PPAccess or not. If not returned then this api will not do any processing
	 * and spit exception. :TODO We could have use @JSonproperty- required=true
	 * to check null, but since fasterxml's version 2.0 onwards,
	 * RequiredProperty field of jsonproperty is not working while
	 * deserializing, current latest fasterxml version 2.3.4 also doesn't
	 * support. there is github issue link @
	...
	private void validatePayPalAccessResponse(PayPalProfile profile) {
		if (StringUtils.isEmpty(profile.getUserId())) {
			...
			throw new PayPalAccessException(
					"user_id is coming as null from PPAccess response",
	...
	protected void configureRestTemplate(RestTemplate restTemplate) {
		...
		FormHttpMessageConverter formMessageConverter = new FormHttpMessageConverter() {
			public boolean canRead(Class<?> clazz, MediaType mediaType) {
				if (mediaType == null
						|| mediaType.compareTo(MediaType.APPLICATION_JSON) == 0) {
					return false;
				}
				// always read non-json as x-www-url-formencoded even though
				// PayPal sets contentType to text/plain
				return true;
			}
		};
	...
	/**
	 * Builds uri for user info service endpoint. Default one given by
	 * {@linkplain PayPalConnectionProperties} will be {@code userInfoUrl} if
	 * null.
	 * 
	...
	private URI buildURI(String accessToken) {
		...
		if (userInfoUrl == null) {
			...
			uriBuilder = URIBuilder.fromUri(PayPalConnectionProperties
					.getUserInfoEndpoint());
		} else {
			uriBuilder = URIBuilder.fromUri(this.userInfoUrl);
		}
		URI returnURI = uriBuilder.queryParam("schema", "openid")
				.queryParam("code", accessToken).build();
		if (logger.isInfoEnabled()) {
